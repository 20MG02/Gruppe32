...
| 3  | Parser für Datei          | JSON-Datei lesen → in `[Record]` umwandeln mit `Data.Aeson.decode` und `map`                        | 2h      | **Gary**       | Nr 2         | —                                                            |
| 4  | CLI-Command-Handler       | `handleCommand :: String -> [String] -> IO ()` für `--insert`, `--delete`, `--filter`, `--query`    | 3h      | **Eslam**      | Nr 1–3       | —                                                            |
| 5  | `--insert`                | Neuen Datensatz in JSON hinzufügen und Datei überschreiben (`encodeFile`)                           | 2h      | **Marco**      | Nr 4         | Ideen zu Validierung oder automatischer ID hier eintragen ⬇️ |
